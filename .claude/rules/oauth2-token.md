...
## 刷新流程
1. 用 refresh_token 查 PostgreSQL
2. 校验 refresh_token 未过期
3. 删除旧的 access_token（PostgreSQL + Redis）
4. 生成新的 access_token
5. 写入 PostgreSQL + Redis
6. 返回新 access_token（refresh_token 不变）

## 注销流程
1. 删除 access_token（PostgreSQL + Redis）
2. 删除关联的 refresh_token（PostgreSQL）
3. 记录登出日志
